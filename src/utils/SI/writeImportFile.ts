...
export const writeImportFile = (currentFile: Series[]) => {
    ...
    const ProgramSheet = currentFile.map(series=>{
        return series.Seasons?.map(season=>{
            return season.map((episode, index)=>{
                ...
                const item = {
                    ['Program Number']: '',
                    'Title': episode.episode_title || `${series.title} S${episodeSeason}E${(index+1).toString().length<2 ? `0${index+1}` : index+1}`,
                    ['Program Type']: 'Series',
                    ['Production Format']: 'A',
    ...
    const dataArrayProgramVersion = ProgramVersion.map((obj: any) => Object.values(obj));
