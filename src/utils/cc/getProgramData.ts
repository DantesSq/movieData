...
const getProgramRequest = async (
  ...
  try {
    ...
    const headers = {
      ...
      const {Title, OriginalNumber, Number, Genres, ProductionYearFrom, ProductionCountries, Duration, PersonsFirms, Channels, Seasons, ProgramVersions} = data
  ...
  //   console.log(ProgramVersionsSPI)
  ...
  // }
      ...
      setUpdatedFile((prev: any) => [
        ...prev,
        {
          ...
          ['EU/NonEU']: data.Attributes[1].Value === 'True' ?'EU' : data.Attributes[3].Value === 'True' ? 'NonEU' : '',
          ['Dependent']: data.Attributes[0].Value === 'True' ?'Dependent' : data.Attributes[2].Value === 'True' ? 'Independent' : '',
...
export const throttleProgramData = async (excelData: ExcelData[], setUpdatedFile: any, setCurrentIndex: any) => {
  let index = 0;
