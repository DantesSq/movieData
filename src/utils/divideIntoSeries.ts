...
export const divideIntoSeries = (data: ExcelData[])=>{
    ...
    for (const index in data) {
        ...
        if (!currentSeriesTitle || !currentSeries) {
            ...
            episodesArr = []
            episodesArr.push({episode_title: episode.episode_title || ``, episode_spi: episode.spi_code, duration: episode.duration, episode_number: Number(index)+1, year: episode.production_year})
            currentSeries.number_of_episodes +=1
            currentSeason = episodeSeason
            currentSeries.year_until = episode.production_year


        } else if (currentSeason != episodeSeason && currentSeriesTitle === episode.series_title) {
