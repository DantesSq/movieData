...
const initialState: LanguagesState = {
  ...
  secondary_data: [
    ...
    { name: "Attributes (test version)", checked: false, key: "attributes" },
  ],
};
...
const LanguagesSlice = createSlice({
  ...
  reducers: {
    setCheckedLanguage: (
      state,
      action: PayloadAction<{ checked: boolean; iso_3166_1: string }>
    ) => {
    ...
    setCheckedMetadata: (
      state,
      action: PayloadAction<{ checked: boolean; name: string }>
    ) => {
...
export const { setCheckedLanguage, setCheckedMetadata } =
